...
counts = np.zeros((len(word_index_dict), len(word_index_dict)), dtype=np.int)
...
probs = normalize(counts.astype(float), norm='l1', axis=1)
...
output_file = open('bigram_generation.txt', 'w')
