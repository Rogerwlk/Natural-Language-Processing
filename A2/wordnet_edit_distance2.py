...
def parse_command_line():
	parser = argparse.ArgumentParser(description='Compute the \
		edit distance between one untagged and one tagged sentence \
		in the SemCor corpus at input indices.')
	parser.add_argument('untagged', help='first sentence index', type=int)
	parser.add_argument('tagged', help='second sentence index', type=int)
...
def sub_cost(word1, word2, sim):
	...
	if sim == 'path':
		similarity = 0
		for synset in word1:
			temp = synset.path_similarity(word2)
			if not temp:
				temp = 0
			similarity = max(similarity, temp)
	elif sim == 'wup':
		similarity = 0
		for synset in word1:
			temp = synset.wup_similarity(word2)
			if not temp:
				temp = 0
			similarity = max(similarity, temp)
	cost = 1 - similarity
...
def wordnet_edit_distance(s1, s2, sim):
	...
	for i, word_i in enumerate(s1):
		synsets_i = wordnet.synsets(word_i)
		for j, chunk_j in enumerate(s2):
			synset_j = semcor_chunk(chunk_j).get_syn_set()
			word_j = semcor_chunk(chunk_j).get_words()
			if synsets_i and synset_j and synset_j in synsets_i or word_i == word_j:
				cmatrix[i + 1][j + 1] = cmatrix[i][j]
				ematrix[i + 1][j + 1] = '='
			else:
				sub = cmatrix[i][j] + sub_cost(synsets_i, synset_j, sim)
				delete = cmatrix[i][j + 1] + del_cost(synsets_i)
